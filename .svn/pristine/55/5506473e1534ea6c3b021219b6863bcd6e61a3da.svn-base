...
class LoginController extends BaseController {
	...
	public function storeLogin() {
		...
		if (filter_var(Input::get('identity'), FILTER_VALIDATE_EMAIL)) {
			...
			if (Profile::where('email', Input::get('identity')) -> count() > 0) {
				//User exist so get email address
				$user = Profile::where('email', Input::get('identity')) -> first();
	...
	public function storeRegister() {
		...
		$input = array( 'email' => Input::get('email'), 'password' => Input::get('password'), 'password_confirmation' => Input::get('password_confirmation'));

		// $user = Sentry::createUser(array(
  //       'email'     => 'alisaheb.doe@example.com',
  //       'password'  => '563679832',        
  //       'activated' => true,
  //   ));

		// Set Validation Rules
		$rules = array('email' => 'required|min:4|max:32|email', 'password' => 'required|min:6|confirmed', 'password_confirmation' => 'required');
